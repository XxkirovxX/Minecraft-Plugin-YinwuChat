...
封禁功能用于禁止指定 Web 账号登录/使用网页端。系统支持以 **账号名或玩家名** 作为目标：

- `/yinwuchat ban <账号名/玩家名> [时长] [理由]`
- 时长支持 `30m / 2h / 1d`，缺省为永久封禁。
- Web 端会弹出封禁确认窗口，游戏内会提示封禁对象、时长与理由。

账号-玩家绑定信息支持查询与解绑（管理员）：

- `/yinwuchat webbind query <账号名/玩家名>`
- `/yinwuchat webbind unbind <账号名/玩家名>`

说明：

- Web 账号与玩家名的自动绑定在 **Web 登录成功并建立 WS 连接且 Token 已绑定** 后完成。
- 使用玩家名执行封禁/查询/解绑时，会自动映射到对应 Web 账号。
